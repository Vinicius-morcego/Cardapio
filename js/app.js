...
var VALOR_ENTREGA = 5;
var CELULAR_EMPRESA = '5534996938988';
// var CELULAR_EMPRESA = '5551996341378';//Denilson
...
cardapio.metodos = {
    //Obtem a lista dos itens do cardapio.
    ...
    //aumentar quantidade do cardapio
    ...
    adicionarAoCarrinho: (id) => {
        ...
        if (qntAtual > 0){ 
            ...
            console.log(JSON.stringify(MEU_CARRINHO));
        }
    },
    //Atualizaçao dos botões de totais do carrinho
    atualizarBadgeTotal: () => {
        var total = 0;
            ...
            if (validaCep.test(cep)){
                ...
                $.getJSON("https://viacep.com.br/ws/"+cep+"/json/?callback=?", function (dados){
                    if (!("erro" in dados)){
                        ...
                        cardapio.metodos.mensagem("CEP não localizado. Preencha as informações manualmente.");
                        $("#txtEndereco").focus();
                    }
                });
            }
    ...
    resumoPedido: () => {
        ...
        cardapio.metodos.carregarEtapa(3);
          console.log(JSON.stringify(MEU_ENDERECO));
    ...
    carregarResumo: () => {
        ...
        $.each(MEU_CARRINHO,(i, e) => {
             let temp = cardapio.templates.itemCarrinhoResumo
                .replace(/\${img}/g,e.img)
                .replace(/\${name}/g,e.name)
                .replace(/\${price}/g,e.price.toFixed(2).replace('.',','))
                .replace(/\${qntd}/g,e.qntd);
    ...
    abrirDepoimento: (depoimento) => {
        ...
        $("#btnDepoimento-2").removeClass("hidden");
        $("#btnDepoimento-3").removeClass("hidden");

        $("#btnDepoimento-"+depoimento).removeClass("hidden");
